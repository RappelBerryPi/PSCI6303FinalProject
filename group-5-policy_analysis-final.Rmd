...
```{r}
glimpse(d)
percentage.control <- percentage(d[number_of_troll_tweets_reported == 0,], d)
percentage.treatment <- percentage(d[number_of_troll_tweets_reported != 0,], d)
percentage.control.changed <- percentage(d[number_of_troll_tweets_reported == 0 & `2024_voting_plan` != general_voting_preference], d[number_of_troll_tweets_reported == 0,])
percentage.treatment.changed <- percentage(d[number_of_troll_tweets_reported > number_of_troll_tweets_interacted_with & `2024_voting_plan` != general_voting_preference], d[number_of_troll_tweets_reported > number_of_troll_tweets_interacted_with,])
```
...
```{r}
mean_tweets <- mean(d$number_of_troll_tweets_seen)
d <- d[,treated := number_of_troll_tweets_reported != 0]
d <- d[,voting_plan_different := `2024_voting_plan` != general_voting_preference]
d <- d[,affected := number_of_troll_tweets_interacted_with > number_of_troll_tweets_reported]
